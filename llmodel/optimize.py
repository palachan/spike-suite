...
def objective(params,Xp,Xe,Xa,Xs,Xahv,Xt,Xn,spike_train,framerate,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego):
    ...
    if smooth:
        ...
        f = penalize_objective(f,params,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego)
...
def penalize_objective(f,params,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego):
    ...
    if params['ego'] is not None:
        if single_ego:
            f += np.sum(ebeta * .5 * np.dot(params['ego'].T, circ_diag) * params['ego'])
        else:
...
def grad(params,Xp,Xe,Xa,Xs,Xahv,Xt,Xn,spike_train,framerate,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego):
    ...
    if smooth:
        #add penalty terms from grouped regularization
        df = penalize_grad(df,params,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego)
...
def penalize_grad(df,params,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego):
    ...
    if params['ego'] is not None:
        if single_ego:
            e_grad = ebeta * np.dot(circ_diag , params['ego'])
        else:
...
def grad_hess(params,args,vector):
    ...
    #unpack arguments
    Xp,Xe,Xa,Xs,Xahv,Xt,Xn,spike_train,framerate,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego = args
    ...
    df1 = line_search_grad(param_vector+r*vector,Xp,Xe,Xa,Xs,Xahv,Xt,Xn,spike_train,framerate,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego)
    df2 = line_search_grad(param_vector-r*vector,Xp,Xe,Xa,Xs,Xahv,Xt,Xn,spike_train,framerate,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego)
...
def make_dict(params,modeltype,single_ego=False):
    ...
    if 'ego' in modeltype:
        if single_ego:
            param_dict['ego'] = params[param_count:param_count+hd_bins]
            param_count += hd_bins
        else:
...
def line_search_grad(params,Xp,Xe,Xa,Xs,Xahv,Xt,Xn,spike_train,framerate,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego):
    ''' convenience function for calculating gradient from param vector '''

    param_dict = make_dict(params,modeltype,single_ego)
    df = grad(param_dict,Xp,Xe,Xa,Xs,Xahv,Xt,Xn,spike_train,framerate,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego)
    
    return df

def line_search_objective(params,Xp,Xe,Xa,Xs,Xahv,Xt,Xn,spike_train,framerate,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego):
    ''' convenience function for calculating objective from param vector '''
    
    param_dict = make_dict(params,modeltype,single_ego)
    f = objective(param_dict,Xp,Xe,Xa,Xs,Xahv,Xt,Xn,spike_train,framerate,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego)
        
    return f


def newton_cg(data, modeltype, betas, single_ego=False, tol=1e-4, maxiter=200, maxinner=30, x0=None):
    """
    ...
    args = (Xp,Xe,Xa,Xs,Xahv,Xt,Xn,spike_train,framerate,modeltype,circ_diag,noncirc_diag,spatial_diag,ego_diag,betas,single_ego)   
    ...
    while k <= maxiter:
        ...
        params = make_dict(param_vector, modeltype, single_ego)
        old_fval = objective(params,*args)
