...
from PySide.QtGui import (QApplication, QMainWindow, QFrame, QLabel, QCheckBox, QLineEdit, QButtonGroup,
...
import spike_analysis.main as main
import spike_analysis.plot as plot
...
class Worker(QObject):
    ...
    #signal for stopping processes (#TODO: not currently functional)
    stop = Signal(str)
    ...
    def load_data(self):
        #start a process
        self.isrunning = True
        main.load_data(self.gui,self.fname)
...
#------------------------------------------------------
    ...
    def set_presets(self):
        ...
        presets.setGeometry(QRect(240, 220, 150, 100))
        ...
        preset_layout.addWidget(self.gridbutton)
        preset_layout.addWidget(self.greatestbutton)
    ...
    def set_options(self):
        ...
        #set geometry and color
        options.setGeometry(QRect(30, 100, 200, 275))
        ...
        self.stbox = QCheckBox('Spike Triggered')
        self.egobox = QCheckBox('Egocentric')
        self.viewbox = QCheckBox('View')
#        self.noveltybox = QCheckBox('Novelty')
        self.centeregobox = QCheckBox('Center Ego')
        self.wallegobox = QCheckBox('Wall Ego')
        #create clear button
        ...
        options_layout.addWidget(self.stbox)
...
#        options_layout.addWidget(self.noveltybox)
    ...
    def set_animations(self):
        ...
        #set geometry and color
        animations.setGeometry(QRect(240, 100, 150, 100))
        ...
        self.ani_hdbox = QCheckBox('HD x Spatial') 
    ...
    def set_labview(self):
        ''' create labview frame '''
        
        #make QFrame
        labview = QFrame(self)
        #make and set label
        labview_label = QLabel()
        labview_label.setText('Run Labview?')
        #set gemoetry and color
        labview.setGeometry(QRect(400, 100, 150, 70))
        labview.setObjectName("labviewWidget")
        labview.setStyleSheet("#labviewWidget {background-color:green;}")
        #create and set layout
        labview_layout = QVBoxLayout()
        labview.setLayout(labview_layout)
        #make labview checkbox
        self.labviewbox = QCheckBox('Of course!') 
        #add to layout
        labview_layout.addWidget(labview_label)        
        labview_layout.addWidget(self.labviewbox)
    ...
    def set_save_options(self):
        ...
        #geometry and color
        save_options.setGeometry(QRect(400,180,150,100))
        ...
        self.loaddatabox = QCheckBox('Load Saved Data')
        #add to layout
        save_layout.addWidget(save_label)
        save_layout.addWidget(self.savebox)
        save_layout.addWidget(self.savedatabox)
        save_layout.addWidget(self.loaddatabox)
    ...
    def set_bonus_options(self):
        ...
        #geometry and color
        bonus_options.setGeometry(QRect(400,290,150,100))
        ...
        self.sumbox = QCheckBox('Make Summary Plots')
        #add to layout
        bonus_layout.addWidget(bonus_label)
        bonus_layout.addWidget(self.sumbox)
    ...
    def set_defaults(self):
        ...
        with open('options.pickle','rb') as f:
            [run_hd,run_spatial,hd_map,run_grid,run_speed,run_ahv,run_autocorr,
             spike_timed,run_ego,run_view,run_center_ego,run_wall_ego,animated_hd_map,heatmap_animation,labview,save_all,
             sumplots,singlecluster,speedmode,savedata,loaddata,acq_system] = pickle.load(f)
        ...
        self.stbox.setChecked(spike_timed) 
        self.egobox.setChecked(run_ego)
        self.viewbox.setChecked(run_view)
#        self.noveltybox.setChecked(run_novelty)
    ...
    def clear_all(self):
        ...
        self.stbox.setCheckState(Qt.Unchecked)
...
#        self.noveltybox.setCheckState(Qt.Unchecked)
...
#        self.noveltybox.setCheckState(Qt.Unchecked)
    ...
    def advanced_settings(self):
        ...
        def save():
            ...
            advanced=[int(self.pic_resolution.text()),int(self.grid_res.text()),
                              int(self.bin_size.text()),float(self.autocorr_width.text()),
                              float(self.framerate.text()),int(self.hd_bins.text()),
                              float(self.sample_cutoff.text())] 
        ...
        def save_as_defaults():
            ...
            #grab settings with appropriate data types
            default_advanced=[int(self.pic_resolution.text()),int(self.grid_res.text()),
                              int(self.bin_size.text()),float(self.autocorr_width.text()),
                              float(self.framerate.text()),int(self.hd_bins.text()),
                              float(self.sample_cutoff.text())] 
        ...
        with open('advanced.pickle','rb') as f:
            [pr,gr,bs,ac,fr,hdb,sc] = pickle.load(f)
        ...
        #give layout
        self.widlayout = QVBoxLayout(self.wid)
        ...
        self.widlayout.addWidget(self.pic_resolution_label)
        self.widlayout.addWidget(self.pic_resolution)
        
        #grid_res
        self.grid_res_label = QLabel()
        self.grid_res_label.setText('Spatial resolution')
        self.grid_res = QLineEdit()
        self.grid_res.setText(str(gr))
        self.widlayout.addWidget(self.grid_res_label)
        self.widlayout.addWidget(self.grid_res)
        
        #bin_size
        self.bin_size_label = QLabel()
        self.bin_size_label.setText('Bin size for temporal analyses (in ms)')
        self.bin_size = QLineEdit()
        self.bin_size.setText(str(bs))
        self.widlayout.addWidget(self.bin_size_label)
        self.widlayout.addWidget(self.bin_size)
        
        #autocorr_width
        self.autocorr_width_label = QLabel()
        self.autocorr_width_label.setText('Spike train autocorrelation width (in seconds)')
        self.autocorr_width = QLineEdit()
        self.autocorr_width.setText(str(ac))
        self.widlayout.addWidget(self.autocorr_width_label)
        self.widlayout.addWidget(self.autocorr_width)
        ...
        self.widlayout.addWidget(self.framerate_label)
        self.widlayout.addWidget(self.framerate)
        
        #hd_bins
        self.hd_bins_label = QLabel()
        self.hd_bins_label.setText('Head direction bins')
        self.hd_bins = QLineEdit()
        self.hd_bins.setText(str(hdb))
        self.widlayout.addWidget(self.hd_bins_label)
        self.widlayout.addWidget(self.hd_bins)
        
        #sample_cutoff
        self.sample_cutoff_label = QLabel()
        self.sample_cutoff_label.setText('Minimum bin occupancy time for spatial (in seconds)')
        self.sample_cutoff = QLineEdit()
        self.sample_cutoff.setText(str(sc))
        self.widlayout.addWidget(self.sample_cutoff_label)
        self.widlayout.addWidget(self.sample_cutoff)
        ...
        self.widlayout.addWidget(self.save)
        self.widlayout.addWidget(self.save_as_defaults)
    ...
    def Run(self):
        ...
        options = [self.hdbox.isChecked(),self.spatialbox.isChecked(),self.hdspatialbox.isChecked(),
                   ...
                   self.timebox.isChecked(),self.stbox.isChecked(),self.egobox.isChecked(),self.viewbox.isChecked(),
                   self.centeregobox.isChecked(),self.wallegobox.isChecked(),self.ani_hdbox.isChecked(),
                   self.ani_heatbox.isChecked(),self.labviewbox.isChecked(),self.savebox.isChecked(),
                   self.sumbox.isChecked(),self.singlebox.isChecked(),self.speedmodebox.isChecked(),
                   self.savedatabox.isChecked(),self.loaddatabox.isChecked(),acq_system]
        ...
        if self.loaddatabox.isChecked():
            #connect the start button to the worker's extra task (calls main.load_data)
            self.start.connect(self.worker.load_data)
        else:
    ...
    def plot_first(self):
        ...
        function = self.metadata['plot_list'][0][3]
        ...
        #set canvas label to name of current cluster (and cam if applicable)
        if self.ops['multi_cam']:
            self.canvas_label.setText(self.trial_data['current_cluster'] + ' camera ' + str(self.trial_data['cam_ids'][self.trial_data['current_cam_ind']]))
        else:
    ...
    def plot_following(self,direction):
        ...
        #accordingly and move on with the plotting procedure
            ...
            #grab the current trial,cluster,and cam from metadata plot_list
            ...
            self.current_cam = self.metadata['plot_list'][self.plot_counter][2]
            
            #grab the plot name and make it into a useable function
            plot_to_make = self.metadata['plot_list'][self.plot_counter][3]
            plot_func = getattr(plot,plot_to_make)
            
            #if the data is ready for this plot (for this cam for this cluster for this trial)...
            if self.metadata[self.current_trial][self.current_cluster][self.current_cam]['dataready'][plot_to_make]:
                #if we're currently calculating data for this cam/cluster/trial in the worker thread...
                if self.current_trial == self.trial_data['current_trial'] and self.current_cluster == self.trial_data['current_cluster'] and self.current_cam == self.trial_data['current_cam_ind']:
                    ...
                    trial_data = self.all_data[self.current_trial][self.current_cluster][self.current_cam]['trial_data']
                    cluster_data = self.all_data[self.current_trial][self.current_cluster][self.current_cam]['cluster_data']
                    spike_data = self.all_data[self.current_trial][self.current_cluster][self.current_cam]['spike_data']
                ...
                #set the canvas label to current cluster (and cam if multi-cam)
                if self.ops['multi_cam']:
                    self.canvas_label.setText(self.current_cluster + ' camera ' + str(trial_data['cam_ids'][self.current_cam]))
                else:
    ...
    def auto_plot(self):
        ...
        for i in range(len(self.metadata['plot_list'])):
            if self.auto:
