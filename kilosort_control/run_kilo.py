        ...
        for fname in ttfiles:
            ...
            if acq == 'neuralynx':
                if trodetype == 'tetrode':
                    waveforms,timestamps,fs,gain = load_nlx.mmap_tt_spike_file(filename)
                elif trodetype == 'stereotrode':
                    waveforms,timestamps,fs,gain = load_nlx.mmap_st_spike_file(filename)
            elif acq == 'openephys':
...
#                print j
    ...
    for trode in range(len(ttfiles)):
        ...
        if trodetype == 'tetrode':
            ...
            kilo2ntt.write_nst(stitched, trode_data[ttfiles[trode]]['spike_times'], trode_data[ttfiles[trode]]['spike_clusters'], kilo_folder, trode+1, fs, gain, acq, ttfiles[trode])
